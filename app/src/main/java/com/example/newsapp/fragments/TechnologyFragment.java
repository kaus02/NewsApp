...
import com.example.newsapp.adapters.TechRecyclerViewAdapter;
import com.example.newsapp.datastore.business.BusinessArticleEntity;
import com.example.newsapp.datastore.technology.MyViewModelFactory;
import com.example.newsapp.datastore.technology.TechArticleEntity;
import com.example.newsapp.datastore.technology.TechArticleViewModel;
...
public class TechnologyFragment extends Fragment implements TechRecyclerViewAdapter.OnRecyclerViewClickListener {
    private  long PAGE_SIZE = 20;
    private long PAGE_INDEX = 1;
    private long TOTAL_ARTICLES;

    private TechArticleViewModel articleViewModel;
    private TechRecyclerViewAdapter adapter;
    private boolean loadMoreData = true;
    private boolean nextRequested = false;
    private boolean isScrolling = false;
    private boolean tabOpened = false;

    private List<TechArticleEntity> articleEntityListMain = new ArrayList<TechArticleEntity>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        articleViewModel = ViewModelProviders.of(this, new MyViewModelFactory(getActivity().getApplication() ,"technology")).get(TechArticleViewModel.class);

        adapter = new TechRecyclerViewAdapter(getContext(),this);
        ...
        articleViewModel.getList().observe(this, new Observer<List<TechArticleEntity>>() {
            @Override
            public void onChanged(List<TechArticleEntity> articleEntities) {
    ...
    private void loadData() {
        ...
        fetchData(new ApiCallback() {
            ...
            public void onSuccess(HeadlineResponse headlineResponse) {
                List<TechArticleEntity> list = new ArrayList<>();
                for(Articles articles : headlineResponse.getArticles()){
                    list.add(new TechArticleEntity(articles.getAuthor(),
                            articles.getTitle(),
                            ...
                            articles.getContent(),
                            articles.getSource().get("name"),"technology"));
                }
                progressBar.setVisibility(View.GONE);
...
//
    ...
    public void onClick(int position, View v) {
        TechArticleEntity article = adapter.getArticleList().get(position);
