...
import com.example.newsapp.adapters.ScienceRecyclerViewAdapter;
import com.example.newsapp.datastore.business.BusinessArticleEntity;
import com.example.newsapp.datastore.science.MyViewModelFactory;
import com.example.newsapp.datastore.science.ScienceArticleEntity;
import com.example.newsapp.datastore.science.ScienceArticleViewModel;
...
public class ScienceFragment extends Fragment implements ScienceRecyclerViewAdapter.OnRecyclerViewClickListener {
    private  long PAGE_SIZE = 20;
    private long PAGE_INDEX = 1;
    private long TOTAL_ARTICLES;

    private ScienceArticleViewModel articleViewModel;
    private ScienceRecyclerViewAdapter adapter;
    private boolean loadMoreData = true;
    private boolean nextRequested = false;
    private boolean isScrolling = false;
    private boolean tabOpened = false;

    private List<ScienceArticleEntity> articleEntityListMain = new ArrayList<ScienceArticleEntity>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        articleViewModel = ViewModelProviders.of(this, new MyViewModelFactory(getActivity().getApplication() ,"science")).get(ScienceArticleViewModel.class);

        adapter = new ScienceRecyclerViewAdapter(getContext(),this);
        ...
        articleViewModel.getList().observe(this, new Observer<List<ScienceArticleEntity>>() {
            @Override
    ...
    private void loadData() {
        ...
        fetchData(new ApiCallback() {
            ...
            public void onSuccess(HeadlineResponse headlineResponse) {
                List<ScienceArticleEntity> list = new ArrayList<>();
                for(Articles articles : headlineResponse.getArticles()){
                    list.add(new ScienceArticleEntity(articles.getAuthor(),
                            articles.getTitle(),
...
//
    ...
    public void onClick(int position, View v) {
        ScienceArticleEntity article = adapter.getArticleList().get(position);
