...
import com.example.newsapp.adapters.SportsRecyclerViewAdapter;
...
import com.example.newsapp.datastore.sports.MyViewModelFactory;
import com.example.newsapp.datastore.sports.SportsArticleEntity;
import com.example.newsapp.datastore.sports.SportsArticleViewModel;
...
public class SportsFragments extends Fragment implements SportsRecyclerViewAdapter.OnRecyclerViewClickListener {
    private  long PAGE_SIZE = 20;
    private long PAGE_INDEX = 1;
    private long TOTAL_ARTICLES;

    private SportsArticleViewModel articleViewModel;
    private SportsRecyclerViewAdapter adapter;
    private boolean loadMoreData = true;
    private boolean nextRequested = false;
    private boolean isScrolling = false;
    private boolean tabOpened = false;

    private List<SportsArticleEntity> articleEntityListMain = new ArrayList<>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        articleViewModel = ViewModelProviders.of(this, new MyViewModelFactory(getActivity().getApplication() ,"sports")).get(SportsArticleViewModel.class);

        adapter = new SportsRecyclerViewAdapter(getContext(),this);
        ...
        articleViewModel.getList().observe(this, new Observer<List<SportsArticleEntity>>() {
            @Override
    ...
    private void loadData() {
        ...
        fetchData(new ApiCallback() {
            ...
            public void onSuccess(HeadlineResponse headlineResponse) {
                List<SportsArticleEntity> list = new ArrayList<>();
                for(Articles articles : headlineResponse.getArticles()){
                    list.add(new SportsArticleEntity(articles.getAuthor(),
                            articles.getTitle(),
...
//
    ...
    public void onClick(int position, View v) {
        SportsArticleEntity article = adapter.getArticleList().get(position);
