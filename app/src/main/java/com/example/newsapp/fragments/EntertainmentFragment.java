...
import com.example.newsapp.adapters.EntertainmentRecylerViewAdapter;
import com.example.newsapp.datastore.business.BusinessArticleEntity;
import com.example.newsapp.datastore.entertainment.EntertainmentArticleEntity;
import com.example.newsapp.datastore.entertainment.EntertainmentArticleViewModel;
import com.example.newsapp.datastore.entertainment.MyViewModelFactory;
...
public class EntertainmentFragment extends Fragment implements EntertainmentRecylerViewAdapter.OnRecyclerViewClickListener{
    private  long PAGE_SIZE = 20;
    private long PAGE_INDEX = 1;
    private long TOTAL_ARTICLES;

    private EntertainmentArticleViewModel articleViewModel;
    private EntertainmentRecylerViewAdapter adapter;
    private boolean loadMoreData = true;
    private boolean nextRequested = false;
    private boolean isScrolling = false;
    private boolean tabOpened = false;

    private List<EntertainmentArticleEntity> articleEntityListMain = new ArrayList<com.example.newsapp.datastore.entertainment.EntertainmentArticleEntity>();
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        articleViewModel = ViewModelProviders.of(this, new MyViewModelFactory(getActivity().getApplication() ,"entertainment")).get(EntertainmentArticleViewModel.class);

        adapter = new EntertainmentRecylerViewAdapter(getContext(),this);
        ...
        articleViewModel.getList().observe(this, new Observer<List<EntertainmentArticleEntity>>() {
            @Override
    ...
    private void loadData() {
        fetchData(new ApiCallback() {
            ...
            public void onSuccess(HeadlineResponse headlineResponse) {
                List<EntertainmentArticleEntity> list = new ArrayList<>();
                for(Articles articles : headlineResponse.getArticles()){
                    list.add(new EntertainmentArticleEntity(articles.getAuthor(),
                            articles.getTitle(),
...
//
    ...
    public void onClick(int position, View v) {
        EntertainmentArticleEntity article = adapter.getArticleList().get(position);
